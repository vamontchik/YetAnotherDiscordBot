...
public sealed class RockPaperScissorsCommand
{
    ...
    {
        if (_isStatCheck)
            await new StatsEntryPoint(_socketCommandContext, _statsManager).DoAsync();
    ...
    private async Task SendErrorMessageAsync() =>
        await _socketCommandContext.Message.ReplyAsync("That's not a rock, paper, or scissors!");
    ...
    private async Task SendGameResultAsync(GameResult gameResult, RpsPlayer bot, RpsPlayer user)
    {
        ...
        await _socketCommandContext.Message.ReplyAsync($"User: {user.Type}, Bot: {bot.Type}, {resultStr}");
    }
    ...
    private RpsPlayer CreateUserPlayer()
    {
        ...
        return new RpsPlayer(userChoiceAsType, _socketCommandContext.User.Username, _socketCommandContext.User.Id);
    }
